...
<body style="margin: 0; overflow: hidden;">
  <a-scene embedded arjs="sourceType: webcam; debugUIEnabled: false;">
    ...
    <!-- Marker 3 for tracking -->
    <a-marker type="pattern" url="{{asset('/images/pattern-marker3.patt')}}" emitevents="true" id="marker2">
      <!-- Plane to display the image -->
      <a-plane class="imagePlane" position="4 0 0" rotation="-90 0 0" width="4" height="2.25" material="shader: flat; scale: 2 2 2"></a-plane>
    </a-marker>
...
<body style="margin: 0; overflow: hidden;">
  ...
  <script>
    ...
    markers.forEach((marker, index) => {
      const imagePlane = imagePlanes[index];
      const imageUrls = [
        "{{ asset('images/non-bio1.jpeg') }}", // Image for marker 1
        "{{ asset('images/non-bio2.jpeg') }}", // Image for marker 2
        "{{ asset('images/non-bio3.jpeg') }}"  // Image for marker 3
      ];
      let isAnimationRunning = false;
      ...
      function updateImageSource() {
        const uniqueParam = Date.now();
        const newImageUrl = `${imageUrls[index]}?v=${uniqueParam}`;
        imagePlane.setAttribute("material", "src", newImageUrl);
      }
